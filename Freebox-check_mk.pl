...
Check_mk like agent for monitoring a Freebox from check_mk.
...
=item -A computer inside your LAN having access to your Freebox on port 443
...
=head1 ÉLÉMENTS REQUIS

=over 12

=item -Le module Perl module WWW::Freebox

=item -Le paquet xinetd

=item -Un ordinateur dans votre LAN ayant accès à votre Freebox sur le port 443
...
=item -Installez le script dans un répeertoire de votre choix sur un ordinateur de votre LAN.

=item -Veuillez-bien lire la section B<IMPORTANT> dans le code, et exécutez les morceaux de code indiqués pour autoriser l'accès à l'API de votre Freebox.

=item -Configurez xinetd comme ci-dessous pour écouter sur le port 6556 (ou un autre, si vous monitorez déjà votre ordinateur auquel cas il faut indiquer le numéro de port à check_mk).

=item -Lancez un inventaire sur votre serveur nagios/check_mk.
